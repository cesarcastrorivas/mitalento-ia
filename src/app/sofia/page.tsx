...
export default function SofiaPage() {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-80px)] max-w-3xl mx-auto px-4 py-6 font-sans">

            {/* ── Header Premium ── */}
            <header className="flex items-center gap-4 mb-6 px-2 py-3 bg-white/60 backdrop-blur-xl border border-white/40 shadow-[0_8px_32px_rgba(0,0,0,0.04)] rounded-3xl sticky top-0 z-10 transition-all">
            ...
            <div className="flex-1 overflow-hidden flex flex-col pt-2">
                ...
                {/* Messages */}
                <div className="flex-1 overflow-y-auto px-2 md:px-6 pb-6 space-y-6 scroll-smooth custom-scrollbar">
                    {messages.map((msg, index) => (
                        ...
                        >
                            ...
                            {msg.role === 'model' && (
                                <div
                                    className="w-8 h-8 rounded-full flex items-center justify-center shrink-0 shadow-sm border border-white/50"
                            ...
                            {/* Bubble */}
                            <div
                                className={`max-w-[80%] px-5 py-3.5 text-[15px] leading-relaxed relative ${msg.role === 'user'
                                    ? 'rounded-[24px] rounded-br-[8px] text-white'
                                    : 'rounded-[24px] rounded-bl-[8px] text-gray-800'
                                    }`}
                                style={{
                                    background: msg.role === 'user'
                                        ? 'linear-gradient(135deg, #6366F1 0%, #8B5CF6 100%)' // primary purple/indigo
                                        : '#ffffff', // white for model
                ...
                {/* ── Input Area Premium ── */}
                <div className="pt-2 pb-1 relative z-10 bg-gradient-to-t from-[var(--bg-main)] via-[var(--bg-main)] to-transparent">
                    <div className="flex items-end gap-2 bg-white rounded-[32px] shadow-[0_8px_32px_rgba(0,0,0,0.06)] border border-[rgba(0,0,0,0.03)] pl-5 pr-2 py-2 focus-within:shadow-[0_8px_32px_rgba(99,102,241,0.12)] focus-within:border-indigo-100 transition-all duration-300">
                        <textarea
                            ref={textareaRef}
                            value={input}
                            ...
                            className="flex-1 my-2.5 text-[15px] text-gray-800 placeholder:text-gray-400 placeholder:font-light resize-none focus:outline-none bg-transparent leading-relaxed custom-scrollbar"
                            rows={1}
                        ...
                        <button
                            ...
                            className={`
                                w-11 h-11 rounded-full flex items-center justify-center shrink-0 transition-all duration-300 transform
                    ...
                    <p className="text-center text-[11px] text-gray-400 mt-4 tracking-wide font-medium opacity-70">
                        Bally es una IA y puede cometer errores. Verifica la información importante.
                    </p>
                </div>
            </div>
